...
kind = "build"
requires = ["db"]
ckb_name = "create_sia_db"
```

# 🧱 Construção do Banco SIA — Base Fiscal e Complementos

Este cookbook cria tabelas auxiliares, índices e estruturas derivadas
necessárias para as análises do SIA.

---

## 1️⃣ Importação de CFOP e tabelas auxiliares

```python
from pathlib import Path
excel_path = SIA_ROOT / "res" / "TrabPaulo.xlsm"

df_cfopd      = pd.read_excel(excel_path, sheet_name="cfopd")
df_regorig    = pd.read_excel(excel_path, sheet_name="regOrig")
df_cfopentsai = pd.read_excel(excel_path, sheet_name="cfopEntSai")
...
## Criação de Índices das tabelas acima importadas
...
### Índice em cfopEntSai
```sql
CREATE INDEX IF NOT EXISTS main.cfopEntSai_cfop_dfe ON cfopEntSai (cfop_dfe);
```

## Criação de Tabelas Adicionais e Índices

### Tabela cfopd_es e índices
```sql
-- tabela derivada unindo cfopd e cfopEntSai
  ...
  SELECT sqA.cfopi, CAST(sqA.g1 AS VARCHAR) AS g1, CAST(sqA.g2 AS VARCHAR) AS g2, CAST(sqA.classe AS VARCHAR) AS classe,
   ...
   FROM (
    SELECT CAST(A.CFOP AS INT) AS cfopi, A.referencia,
      ...
      FROM _fiscal_ComparacaoGiaEfdPorCfop AS A
...
### idDocAtributos_compl - Tudo o que preciso para complementar docatrib_fiscal_DocAtributos, como tp_origem, origem, CNPJ, UF e Razão social do Participante, chave, numero e NatOp

#### Observações não estão aqui porque em algumas empresas é uma tabela que ocupa um espaço enorme. Por isso, alternei para idDocAtributos_chaveObs

QUEM É O PARTICIPANTE?
- indEmit	= 0  indOper = 0    E    Part = C.idDest
- indEmit	= 0  indOper = 1    S    Part = C.idDest
...
LEFT OUTER JOIN docatrib_fiscal_DocAtributosPart AS C ON C.idDocAtributos = A.idDocAtributos
LEFT OUTER JOIN _fiscal_ParticipanteDeclarado AS F ON F.idParticipanteDeclarado = C.idEmit
LEFT OUTER JOIN _fiscal_ParticipanteDeclarado AS G ON G.idParticipanteDeclarado = C.idDest
LEFT OUTER JOIN dfe_fiscal_NfeC100 AS H1 ON H1.idNFeC100 = A.idRegistro
LEFT OUTER JOIN dfe_fiscal_DocNum AS Q ON Q.idRegistro = A.idRegistro AND Q.idRegistroDeOrigem = A.idRegistroDeOrigem
```
...
### idDocAtributos_chaveObs ##
onde há chave, relaciona chave com idDocAtributos (e vice-versa) e observações

#### (##TODO - Obs em SAT e CTe) - Por isso que no SQL está lá J1 e L1
Como é por chave, somente pega tp_origem = 'DFe', senão duplica!!!
...
LEFT OUTER JOIN regOrig AS B ON B.idRegistroDeOrigem = A.idRegistroDeOrigem
LEFT OUTER JOIN dfe_fiscal_NfeC100 AS H1 ON H1.idNFeC100 = A.idRegistro
LEFT OUTER JOIN dfe_fiscal_NfeC110 AS H2 ON H2.idNfeC100 = H1.idNfeC100
LEFT OUTER JOIN dfe_fiscal_NfeC200 AS J1 ON J1.idNfeC200 = A.idRegistro
LEFT OUTER JOIN dfe_fiscal_Sat100 AS L1 ON L1.idSat100 = A.idRegistro
...
## 🧱complementos de apuração e itens para idDocAtributos (muitos para um)

### idDocAtributos_cfops ##
os G1s estava duplicando... Exemplo: ChaveX 1 Receitas CFOP Industria e ChaveX 1 Receitas CFOP Comercio por isso tive que fazer duas vezes
ver Sqlite_Audits_BaseCv.php - originalmente usava cfopEntSai, eu desativei
...
SELECT idDocAtributos, GROUP_CONCAT(g1, '|') AS g1s, GROUP_CONCAT(valorDaOperacao, '|') AS valoresDaOperacao
  ...
  (SELECT AO.idDocAtributos, AO.cfop,
    ...
    FROM DocAtrib_fiscal_DocAtributosDeApuracao AS AO
    LEFT OUTER JOIN DocAtrib_fiscal_DocAtributos AS A ON A.idDocAtributos = AO.idDocAtributos
...
```sql
-- o sql abaixo é estranho... porque fiz duas vezes? Porque pode haver alíquotas diferentes. Na primeira, agrupa por exemplo 5102 12% 5102 18% 5901 0% para 5102 e 5901 . Aí na segunda concatena em cfops
-- ver Sqlite_Audits_BaseCv.php - originalmente usava cfopEntSai, eu desativei
...
SELECT idDocAtributos, GROUP_CONCAT(classif, '') AS classifs
  ...
  (SELECT idDocAtributos, '[' || CAST(idClassificacao AS VARCHAR) || ']' AS classif
    FROM docatrib_fiscal_DocClassificado
...
FROM DocAtrib_fiscal_DocAtributos AS A
INNER JOIN dfe_fiscal_Evt191 AS H ON H.idNFeC100 = A.idRegistro
LEFT OUTER JOIN dfe_fiscal_Evt192 AS I ON I.idEvt191 = H.idEvt191
...
FROM DocAtrib_fiscal_DocAtributos AS A
INNER JOIN dfe_fiscal_Evt291 AS J ON J.idNfeC200 = A.idRegistro
LEFT OUTER JOIN dfe_fiscal_Evt292 AS K ON K.idEvt291 = J.idEvt291
  ...
  (SELECT sqC.idDocAtributos AS idDocAtributos, CASE WHEN sqC.cidDA <= 3 THEN descrisBig ELSE descrisShort END AS descris FROM
    ...
    (SELECT * FROM
      (SELECT A.idDocAtributos, SUBSTR(B.descricao, 1, 50) AS descriBig, SUBSTR(B.descricao, 1, 25) AS descriShort
        FROM docatrib_fiscal_DocAtributosItem AS A
        LEFT OUTER JOIN _fiscal_ItemServicoDeclarado AS B ON B.idItemServicoDeclarado = A.idItemServicoDeclarado) AS sqA
...
SELECT idDocAtributos, GROUP_CONCAT(COD_NCM, '|') AS codncms FROM
  (SELECT * FROM
    (SELECT A.idDocAtributos, A.COD_NCM
      FROM docatrib_fiscal_DocAtributosItem AS A) AS sqA
...
SELECT idDocAtributos, GROUP_CONCAT(aliqIcms, '|') AS aliqs FROM
  (SELECT * FROM
    (SELECT A.idDocAtributos, A.aliqIcms
      FROM DocAtrib_fiscal_DocAtributosDeApuracao AS A) AS sqA
...
FROM _fiscalCadesp_HistoricoRegimeDeApuracao AS A
...
FROM _fiscalCadesp_Cadastro AS A
LEFT OUTER JOIN _fiscal_OcorrenciaCadesp AS B ON B.cdOcorrenciaCadastral = A.cdOcorrenciaCadastral;
```
...
#### 🧱docAtribTudao para complementar docatrib_fiscal_DocAtributos com todas essas tabelas preparatórias acima
...
FROM DocAtrib_fiscal_DocAtributos AS B
...
LEFT OUTER JOIN dfe_fiscal_NfeC100 AS H ON H.idNFeC100 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_EfdC100 AS I ON I.idEfdC100 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_NfeC200 AS J1 ON J1.idNfeC200 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_NfeC211Resumo AS J2 ON J2.idNfeC200 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_EfdD100 AS K ON K.idEfdD100 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_Sat100 AS L1 ON L1.idSat100 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_Sat104 AS L2 ON L2.idSat100 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_EfdC800 AS M ON M.idEfdC800 = B.idRegistro
LEFT OUTER JOIN dfe_fiscal_NfceC100 AS N ON N.idNFCeC100 = B.idRegistro
```
...
#### 🧱chaveNroTudao para juntar docatrib_fiscal_DocAtributos em uma linha apenas, uma linha para cada chave de acesso
...
LEFT OUTER JOIN DocAtrib_fiscal_DocAtributos AS DA_Efd ON DA_Efd.idDocAtributos = SqB.EFD_idDocAtributos
LEFT OUTER JOIN docAtribTudao AS DA_EfdT ON DA_EfdT.idDocAtributos = DA_Efd.idDocAtributos
LEFT OUTER JOIN DocAtrib_fiscal_DocAtributos AS DA_DFe ON DA_DFe.idDocAtributos = SqB.DFe_idDocAtributos
...
### Índices usados no cookbook conc.md

#### ConciliacaoEfdNFe_pre
...
### Tabelas e Índices usados no cookbook an_econ.md

#### Tabela entsaicnpj e seu índice
...
SELECT
  ...
  -- ⚠️ quanto às datas, se for EFD, sempre tem dtEntSd, mas NFe nem sempre tem, por isso joguei dtEmissao
  ...
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'EFD' THEN -AO.valorDaOperacao 
    WHEN B.tp_origem = 'EFD' THEN AO.valorDaOperacao 
    ELSE 0 
  END AS es_valconEFD,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'EFD' THEN -AO.bcIcmsOpPropria 
    WHEN B.tp_origem = 'EFD' THEN AO.bcIcmsOpPropria 
    ELSE 0 
  END AS es_bcicmsEFD,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'EFD' THEN -AO.icmsProprio 
    WHEN B.tp_origem = 'EFD' THEN AO.icmsProprio 
    ELSE 0 
  END AS es_icmsEFD,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'EFD' THEN -AO.bcIcmsSt 
    WHEN B.tp_origem = 'EFD' THEN AO.bcIcmsSt 
    ELSE 0 
  END AS es_bcicmsstEFD,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'EFD' THEN -AO.icmsSt 
    WHEN B.tp_origem = 'EFD' THEN AO.icmsSt 
    ELSE 0 
  END AS es_icmsstEFD,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'DFe' THEN -AO.valorDaOperacao 
    WHEN B.tp_origem = 'DFe' THEN AO.valorDaOperacao 
    ELSE 0 
  END AS es_valconDFe,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'DFe' THEN -AO.bcIcmsOpPropria 
    WHEN B.tp_origem = 'DFe' THEN AO.bcIcmsOpPropria 
    ELSE 0 
  END AS es_bcicmsDFe,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'DFe' THEN -AO.icmsProprio 
    WHEN B.tp_origem = 'DFe' THEN AO.icmsProprio 
    ELSE 0 
  END AS es_icmsDFe,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'DFe' THEN -AO.bcIcmsSt 
    WHEN B.tp_origem = 'DFe' THEN AO.bcIcmsSt 
    ELSE 0 
  END AS es_bcicmsstDFe,
  CASE 
    WHEN EI.cfop < 5000 AND B.tp_origem = 'DFe' THEN -AO.icmsSt 
    WHEN B.tp_origem = 'DFe' THEN AO.icmsSt 
    ELSE 0 
  END AS es_icmsstDFe,
  EI.dfi, EI.st, EI.classe, EI.g1, EI.c3, EI.g2, EI.g3, EI.descri_simplif
FROM
  DocAtrib_fiscal_DocAtributosDeApuracao AS AO
  LEFT OUTER JOIN DocAtrib_fiscal_DocAtributos AS A ON A.idDocAtributos = AO.idDocAtributos
  LEFT OUTER JOIN docAtribTudao AS B ON B.idDocAtributos = A.idDocAtributos
...
### Tabelas e Índices usados no cookbook safic_menu.md

#### Índices necessários pra acelerar
